...
export default function HostelDetailsPage() {
  ...
  const fetchTenants = async () => {
    if (!isLoaded || !user || !params.hostelId) return;
    try {
      const response = await fetch(`/api/users/${user.id}/tenants?hostelId=${params.hostelId}`,
        {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fetchHostelDetails = async () => {
      if (!isLoaded || !user || !params.hostelId) return;
      try {
        const response = await fetch(`/api/users/${user.id}/hostel/${params.hostelId}`,
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  }, [params.hostelId, user?.id, isLoaded]);

  const handleTenantCreated = () => {
    if (isLoaded && user && params.hostelId) {
      fetchTenants();
    }
  };

  const handleRefreshPayments = async () => {
    if (!params.hostelId) return;
    ...
    try {
      const response = await fetch('/api/rent-payments/refresh', {
        ...
        body: JSON.stringify({ hostelId: params.hostelId }),
      });
  ...
  const handleSettingsClick = () => {
    router.push(`/dashboard/${params.hostelId}/settings`);
  };

  const handleViewDetails = (tenantId: string) => {
    router.push(`/dashboard/${params.hostelId}/tenants/${tenantId}`);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-6">
        <h1 className="text-3xl font-bold">{hostel?.name || "Loading..."}</h1>
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <CreateTenantSheet
            hostelId={Array.isArray(params.hostelId) ? params.hostelId[0] : params.hostelId}
