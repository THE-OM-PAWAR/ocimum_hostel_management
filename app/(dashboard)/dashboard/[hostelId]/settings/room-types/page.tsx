...
export default function RoomTypesPage() {
  ...
  const fetchRoomTypes = async () => {
    try {
      const response = await fetch(`/api/hostels/${params.hostelId}/room-types`);
  ...
  const handleDelete = async (roomTypeId: string) => {
    ...
    try {
      await fetch(`/api/hostels/${params.hostelId}/room-types/${roomTypeId}`, {
        method: "DELETE",
      });
      toast({
        title: "Success",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Manage and organize room types for your organisation
        </p>
      </div>
