...
export default function HostelProfilePage() {
  ...
  const fetchHostelProfile = async () => {
    if (!user?.id || !params.hostelId) return;
    
    try {
      // First get basic hostel info
      const hostelResponse = await fetch(`/api/users/${user.id}/hostel/${params.hostelId}`);
      const hostelData = await hostelResponse.json();
      setHostelInfo(hostelData);
      
      // Then get hostel profile
      const profileResponse = await fetch(`/api/hostels/${params.hostelId}/profile`);
      
      if (profileResponse.ok) {
        const profileData = await profileResponse.json();
        setProfile(profileData);
      } else {
        // If no profile exists, try to get organisation profile data
        try {
          ...
          let organisationProfile: HostelProfile | null = null;
          if (!organisationData.error && organisationData.organisationId) {
            const organisationProfileResponse = await fetch(`/api/organisations/${organisationData.organisationId}/profile`);
            if (organisationProfileResponse.ok) {
              organisationProfile = await organisationProfileResponse.json();
            }
          }
          
          // Use organisation profile data as fallback
          setProfile(prev => ({
            ...prev,
            basicInfo: {
              ...prev.basicInfo,
            ...
            propertyDetails: {
              ...
              accommodationType: organisationProfile?.propertyDetails?.accommodationType || 'boys',
            },
            locationInfo: {
              ...prev.locationInfo,
              googleMapLink: organisationProfile?.locationInfo?.googleMapLink || "",
              nearbyLandmarks: organisationProfile?.locationInfo?.nearbyLandmarks || [],
              transportConnectivity: [],
            },
          }));
        } catch (error) {
          console.error("Error fetching organisation profile:", error);
          // Use default with hostel name
          setProfile(prev => ({
            ...prev,
            basicInfo: {
              ...prev.basicInfo,
      ...
      toast({
        title: "Error",
        description: "Failed to load hostel profile",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: "Success",
        description: "Hostel profile updated successfully",
      });
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to save hostel profile",
        variant: "destructive",
      });
    } finally {
      setSaving(false);
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Manage detailed information about your hostel
        </p>
      </div>
      ...
      <div className="flex justify-end">
        <div className="flex gap-3">
          <Button
            ...
            onClick={async () => {
              try {
                const response = await fetch(`/api/hostels/${params.hostelId}/profile/auto-populate`);
                if (response.ok) {
                  const data = await response.json();
                  setProfile(data);
                  toast({
                    title: "Success",
                    description: "Profile auto-populated from organisation data",
                  });
                } else {
                ...
                toast({
                  title: "Error",
                  description: "Failed to auto-populate from organisation profile",
                  variant: "destructive",
                });
              }
            }}
          ...
          >
            Auto-populate from Organisation
          </Button>
