...
export default function HostelSettingsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Hostel Settings</h1>
        <p className="text-muted-foreground mt-2">
          Configure and customize your hostel settings
        </p>
      </div>

      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {settingsOptions.map((option, index) => (
          <Link key={option.href} href={`/dashboard/${hostelId}/settings/${option.href}`}>
            <motion.div
