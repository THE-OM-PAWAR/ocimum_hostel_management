...
function CreateBlockCard({ onClick }: { onClick: () => void }) {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      className="bg-gradient-to-br from-primary/5 to-primary/10 p-6 rounded-lg border-2 border-dashed border-primary/20 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer flex flex-col items-center justify-center h-[200px] group"
      onClick={onClick}
    >
      ...
      <div className="absolute inset-0 rounded-lg bg-gradient-to-br from-primary/0 to-primary/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
    </motion.div>
  );
}
...
function BlockCard({ index, name, description, id }: BlockCardProps) {
  ...
  return (
    <motion.div
      ...
      className="bg-card p-6 rounded-lg border shadow-sm hover:shadow-md transition-all duration-300 h-[200px] flex flex-col cursor-pointer group"
